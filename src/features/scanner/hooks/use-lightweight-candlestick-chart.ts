...
export function useLightweightCandlestickChart({
  ...
  useEffect(() => {
    ...
    const priceSeries = createPriceSeries(chart, initialData, initialChartType);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const priceSeries = createPriceSeries(chart, latestDataRef.current, chartType);
      priceSeriesRef.current = priceSeries;
...
function createPriceSeries(
  ...
  if (chartType === "bar-ohlc") {
    const series = chart.addSeries(BarSeries, createScannerBarSeriesOptions());
    series.setData(data.barRenderData);
    return series;
  }

  if (chartType === "bar-hlc") {
    const series = chart.addSeries(BarSeries, createScannerHlcBarSeriesOptions());
    series.setData(data.hlcBarRenderData);
    return series;
  }

  if (chartType === "line") {
    const series = chart.addSeries(LineSeries, createScannerLineSeriesOptions());
  ...
  if (chartType === "step-line") {
    const series = chart.addSeries(LineSeries, createScannerStepLineSeriesOptions());
    series.setData(data.lineRenderData);
    return series;
  }
  ...
  const series = chart.addSeries(CandlestickSeries, createScannerCandleSeriesOptions());
