    ...
    >
      <div className="sm:hidden">
        <ScannerIconButton
          label="Drawing tools"
          icon={PenLine}
          active={mobileSheetOpen || hasSelectedDrawing}
      ...
      <Sheet open={mobileSheetOpen} onOpenChange={setMobileSheetOpen}>
        ...
        >
          ...
          <div className="flex items-center justify-around gap-1 rounded-md border border-border bg-background/35 p-1">
            ...
            <ShareMenu stock={stock} compact />
          </div>
          <div className="flex max-h-[55dvh] flex-col gap-3 overflow-y-auto pr-1">
            {groups.map((group) => (
              <div key={group.id} className="rounded-md border border-border bg-background/35 p-1">
                <div className="px-2 pb-1 pt-1 text-xs font-bold text-foreground">{group.label}</div>
