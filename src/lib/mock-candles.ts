import type { Candle, MeasurementBox, ScanBand } from "@/types/market";

const SEED = 20250106;
// Must stay >= 250 so the near-250-week-high scanner (src/lib/scanners/near-250-week-high.ts)
// has a full lookback window to evaluate.
...
// RALLY_WINDOWS were originally authored against a 160-candle series, with the last
// window ending near the final candle. Shifting every start by this offset keeps that
// same "rally into the most recent candle" shape as NUM_CANDLES grows.
...
// Max ScanBands rendered at once — keeps the chart from turning into a wall
// of repeated vertical bands; only the most significant rallies get one.
const MAX_SCAN_BANDS = 3;
...
function computeWindowStats(candles: Candle[]): WindowStats[] {
  return RALLY_WINDOWS.map((window, index) => {
    ...
    return { window, index, slice, lowPrice, highPrice, percentValue };
  });
}

// Only the MAX_SCAN_BANDS most significant rallies (by % gain) become a
// narrow full-height ScanBand — "meaningful breakout periods", not every
// detected rally. Window lengths (8-12 candles) stay well within the
// "8 to 20 candles" visual-width guideline; never derive a highlight from
// the full 250-week calculation lookback.
...
// Every rally window still gets a MeasurementBox — these are the detailed
// price-range readouts, distinct from the (fewer) headline ScanBands above.
function deriveMeasurementBoxes(stats: WindowStats[]): MeasurementBox[] {
  return stats.map(({ window, index, slice, lowPrice, highPrice, percentValue }) => {
    const gain = highPrice - lowPrice;
    const bars = window.length * 7;
    ...
    return {
      ...
      label: "Scale",
      percent: `${gain.toFixed(2)} (${percentValue.toFixed(2)}%)`,
      bars: `${window.length} bars, ${bars}d`,
    };
  });
}
...
export const mockMeasurementBoxes: MeasurementBox[] = deriveMeasurementBoxes(windowStats);
