...
import type { DashboardCardData, DashboardItemColor } from "@/lib/mock-dashboard";
...
export function DashboardWidget({ card }: { card: DashboardCardData }) {
  ...
  return (
    <div className="min-w-0 flex-1 px-3 py-3">
      ...
      <p className="mt-0.5 text-[11px] text-muted-foreground">{card.timestamp}</p>
      ...
      <div className="mt-2.5 flex flex-col">
        {card.items.map((item) => {
          ...
          const valueClass = isPositive ? "text-brand-green" : "text-brand-red";

          return (
            <div key={item.rank} className="flex items-center gap-2 py-0.5">
              <span className="w-22 shrink-0 truncate text-[11px] text-foreground">
                {item.label}
              </span>
              ...
              <span
                className={`w-12 shrink-0 text-right text-[11px] font-medium tabular-nums ${valueClass}`}
              >
